...
{
    public partial class BasicInfoTelephoneUpdate : DevExpress.XtraEditors.XtraUserControl
    {
        public BasicInfoTelephoneUpdate(int room_id)
        {
            InitializeComponent();
            DataTable roomDetail = BusinessLogicBridge.DataStore.getPhoneById(room_id);

            string coderef = roomDetail.Rows[0]["coderef"].ToString();
            string phone_label = roomDetail.Rows[0]["phone_label"].ToString();
            string phone_id = roomDetail.Rows[0]["phone_id"].ToString();


            textEditMeterId.EditValue = phone_id;

            txtmeter_label.Properties.ReadOnly = true;
            txtmeter_label.EditValue = phone_label;
        ...
        private void simpleButton2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        BusinessLogicBridge.DataStore.updateDataPhone(lookUpEditBuilding.EditValue.ToString(), lookUpEditFloor.EditValue.ToString(), gridLookUpEditRoom.EditValue.ToString(), txtmeter_label.Text, txtmeter_serial.Text, txtmeter_model.Text, memometer_detail.Text, Convert.ToInt16(textEditMeterId.Text));
                        BasicInfoTelephone.AddPanel_ControlRemoved();
                        BasicInfoTelephone.UpdatePanel.Close();
                    }
                }
            }
        }
