...
{   
    public partial class tenant_cancelbooking : DevExpress.XtraReports.UI.XtraReport
    {

        public tenant_cancelbooking()
        {
            InitializeComponent();
        }
        ...
        public void loopGenDataRow(DataTable roomTable, string Building, string roomFrom, string roomTo)
        {
            ...
            x.Columns.Add("reserve_end_date", typeof(DateTime));
            ...
            try
            {
                ...
                {
                    x.Rows.Add(roomTable.Rows[i]["coderef"], roomTable.Rows[i]["room_label"], roomTable.Rows[i]["tenant_name"].ToString() + " " + roomTable.Rows[i]["tenant_surname"].ToString(), roomTable.Rows[i]["tenant_phone"].ToString(), roomTable.Rows[i]["tenant_mobile"].ToString(), roomTable.Rows[i]["reserve_create_date"], roomTable.Rows[i]["reserve_end_date"]);
                }
            }
            catch(Exception ex) {
            ...
            //RoomDS.WriteXml(@"C:\tenantCancelSourceSchema.xml", System.Data.XmlWriteMode.WriteSchema);
        }
    }
}
