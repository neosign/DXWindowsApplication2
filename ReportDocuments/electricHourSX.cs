...
{   
    public partial class electricHourSX : DevExpress.XtraReports.UI.XtraReport
    {

        public electricHourSX()
        {
            InitializeComponent();
        ...
        public void loopGenDataRow(DataTable roomTable, string Building, int roomFrom, int roomTo, DateTime day)
        {
            ...
            ETransByDay.Columns.Add("current", typeof(double));
            ...
            for (int i = 0; i < roomTable.Rows.Count; i++)
            {
                ...
                ReportDTTemp = BusinessLogicBridge.DataStore.getReportEHourSX(day, RoomDT.Rows[0]["meter_serial"].ToString());

                for (int j = 0; j < ReportDTTemp.Rows.Count; j++){

                    ETransByDay.Rows.Add(ReportDTTemp.Rows[j]["TimeRecord"].ToString(), ReportDTTemp.Rows[j]["TotalUnit"].ToString(), ReportDTTemp.Rows[j]["Current_Main"].ToString());
            ...
            //RoomDS.WriteXml(@"C:\electricHourSXSchema.xml", System.Data.XmlWriteMode.WriteSchema);
        }
    }
}
