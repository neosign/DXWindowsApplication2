...
{
    public partial class waterHourUnit : DevExpress.XtraReports.UI.XtraReport
    {

        public waterHourUnit()
        {
            InitializeComponent();
        ...
        public void loopGenDataRow(DataTable roomTable, string Building, int roomFrom, int roomTo, DateTime day)
        {
            ...
            ChartDT.Columns.Add("hour_num", typeof(int));
            ...
            for (int i = 0; i < roomTable.Rows.Count; i++)
            {
                ...
                ReportDTTemp = BusinessLogicBridge.DataStore.getReportWHourlyUnit(day, RoomDT.Rows[0]["meter_serial"].ToString());
            ...
            {
                ChartDT.Rows.Add(ETransByDay.Rows[y]["hour_num"].To<int>(), ETransByDay.Rows[y]["hour_range"].ToString(), ETransByDay.Rows[y]["total"].To<double>());
            }

            DataView dv = ChartDT.DefaultView;
            dv.Sort = "hour_num asc";
            DataTable sortedDT = dv.ToTable();

            xrChart1.DataSource = sortedDT;
            ...
            //RoomDS.WriteXml(@"C:\waterUnitHourSchema.xml", System.Data.XmlWriteMode.WriteSchema);
        }
    }
}
