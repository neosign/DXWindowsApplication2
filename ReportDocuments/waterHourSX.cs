...
{   
    public partial class waterHourSX : DevExpress.XtraReports.UI.XtraReport
    {
        public waterHourSX()
        {
            InitializeComponent();
        ...
        public void loopGenDataRow(DataTable roomTable, string Building, int roomFrom, int roomTo, DateTime day)
        {
            ...
            for (int i = 0; i < roomTable.Rows.Count; i++)
            {
                ...
                ReportDTTemp = BusinessLogicBridge.DataStore.getReportWHour(day, RoomDT.Rows[0]["meter_serial"].ToString());

                for (int j = 0; j < ReportDTTemp.Rows.Count; j++){

                    ETransByDay.Rows.Add(ReportDTTemp.Rows[j]["TimeRecord"].ToString(), ReportDTTemp.Rows[j]["TotalUnit"].ToString());
            ...
            RoomDS.WriteXml(@"C:\waterHourSchema.xml", System.Data.XmlWriteMode.WriteSchema);
        }
    }
}
