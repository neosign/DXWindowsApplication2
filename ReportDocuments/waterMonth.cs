...
{   
    public partial class waterMonth : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        public void loopGenDataRow(DataTable roomTable, string Building, int roomFrom, int roomTo, DateTime monthFrom, DateTime monthTo)
        {
            ...
            for (int i = 0; i < roomTable.Rows.Count; i++)
            {
                ...
                //ReportDTTemp = BusinessLogicBridge.DataStore.getW_ReportMonthly(RoomDT.Rows[0]["meter_serial"].ToString(), monthFrom, monthTo);
                ReportDTTo = BusinessLogicBridge.DataStore.getW_ReportMaxMinKwh(RoomDT.Rows[0]["meter_serial"].ToString(), monthFrom);
            ...
            RoomDS.WriteXml(@"C:\waterMonthSchema.xml", System.Data.XmlWriteMode.WriteSchema);
        }
    }
}
