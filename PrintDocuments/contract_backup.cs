...
{
    public partial class contract_backup : DevExpress.XtraReports.UI.XtraReport
    {
        public contract_backup()
        {
            InitializeComponent();
        }
        ...
        public void loopGenDataRow(int check_in_id) {
            ...
            xrTableCellContactNO.Text   = CheckInData.Rows[0]["check_in_label"].ToString();
            xrTableCellCompanyName.Text = CheckInData.Rows[0]["company_name"].ToString();
            xrTableCellCreateDate.Text = Convert.ToDateTime(CheckInData.Rows[0]["check_in_date"].ToString()).ToString(MainForm.dateformat);
            xrTableCellOwnerName.Text = (CheckInData.Rows[0]["company_owner_name"].ToString() == "") ? "-" : CheckInData.Rows[0]["company_owner_name"].ToString();
            xrTableCellTenantName.Text = CheckInData.Rows[0]["tenant_name"].ToString() + " " + CheckInData.Rows[0]["tenant_surname"].ToString();
            xrTableCellCompanyAddress.Text = CheckInData.Rows[0]["company_address"].ToString();
            ...
            string Topic3 = xrLabelTopic3.Text;

            Topic3 = Topic3.Replace("[depnum]", CheckInData.Rows[0]["roomtype_month_insure_price"].To<double>().ToString("N2"));

            Topic3 = Topic3.Replace("[deptext]", MainForm.ThaiBaht(CheckInData.Rows[0]["roomtype_month_insure_price"].ToString()));

            xrLabelTopic3.Text = Topic3;
